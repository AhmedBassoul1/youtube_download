...
def run_download_task(job_id: str, url: str, is_audio: bool):
    ...
    try:
        ...
        # Get command
        if is_audio:
            full_cmd = cmd_engine.get_command_audio(output_dir, url)
        else:
            full_cmd = cmd_engine.get_command_video(output_dir, url)
...
async def start_download(request: DownloadRequest, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(run_download_task, job_id, request.url, request.is_audio)
