...
class DownloadRequest(BaseModel):
    ...
    selected_indices: Optional[List[int]] = None  # 1-based indices for playlist items
...
async def get_playlist_info(url: str):
    ...
    result = dl_engine.get_playlist_videos(url)
    if not result:
        return {"is_playlist": False, "videos": []}
    return {"is_playlist": True, **result}
...
def run_download_task(job_id: str, url: str, is_audio: bool, quality: str,
                      output_dir: Optional[str], selected_indices: Optional[List[int]] = None):
    ...
    try:
        ...
        playlist_items = selected_indices if selected_indices else None

        if is_audio:
            full_cmd = cmd_engine.get_command_audio(full_output_dir, url, quality=quality, playlist_items=playlist_items)
        else:
            full_cmd = cmd_engine.get_command_video(full_output_dir, url, quality=quality, playlist_items=playlist_items)
